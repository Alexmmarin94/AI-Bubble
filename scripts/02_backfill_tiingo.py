...
DEFAULT_BACKFILL_START = "2010-01-01"
...
def fetch_tiingo_eod(
    ...
    """
    Fetch Tiingo EOD prices for a ticker.
    GET /tiingo/daily/{ticker}/prices?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD&token=...
    """
...
def main() -> None:
    ...
    cfg = read_yaml(repo_root / "config" / "tiingo_universe.yml")
    tickers = parse_tickers(cfg)

    if not tickers:
        raise RuntimeError(
            "No tickers found in config/tiingo_universe.yml under key 'tickers'. "
            "Example:\n"
            "tickers:\n  - SPY\n  - QQQ\n"
        )

    # Optional full refresh switch
    full_refresh = os.getenv("TIINGO_FULL_REFRESH", "0").strip() == "1"

    out_dir = repo_root / "data" / "raw" / "tiingo"
    ensure_dir(out_dir)
    ...
    for ticker, cfg_backfill_start in tickers:
        ...
        if last_dt is None:
            start_date = cfg_backfill_start or DEFAULT_BACKFILL_START
        else:
            start_date = (last_dt + pd.Timedelta(days=1)).strftime("%Y-%m-%d")

        df_new = fetch_tiingo_eod(session, token, ticker, start_date=start_date)
        ...
        print(f"[OK] Tiingo {ticker} - rows={len(df_all)} last_date={last_date}")
